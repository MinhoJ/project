...
public class CartRestController {
	...
	// ~
	@RequestMapping(value = "/addquantity/{productId}", method = RequestMethod.POST)
	public ResponseEntity<Void> addQunatity(@PathVariable(value = "productId") int productId) {
		...
		if (product.getUnitInStock() > quantity) {
			...
			cartitem.setTotalPrice(product.getPrice() * cartitem.getQuantity());
			cartItemService.addQuantity(cartitem);
		}
	...
	@RequestMapping(value = "/substractquantity/{productId}", method = RequestMethod.POST)
	public ResponseEntity<Void> substractQunatity(@PathVariable(value = "productId") int productId) {
		...
		if (0 < cartItem.getQuantity()) {
			cartItem.setQuantity(quantity - 1);
			cartItem.setTotalPrice(product.getPrice() * cartItem.getQuantity());
			cartItemService.substractQuantity(cartItem);
		} else
			return new ResponseEntity<Void>(HttpStatus.NO_CONTENT);
